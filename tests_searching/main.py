...
def main():
    ...
    for mode in mode_search:
        ...
        try:
            index = faiss.read_index(rf"C:\Users\rudkevich-k\Desktop\patent_diploma\pre_compute_embeddings_for_search\indexs\{mode}_index.bin")
            

            print("Извлечение векторов из FAISS...")
            # reconstruct_n работает быстрее, чем цикл reconstruct
            test_vectors = np.zeros((total_samples, 896), dtype=np.float32)
        ...
        checkpoint_file = f"{mode}_results_of_{args.experiment}_cache.pkl"
        ...
        df_result.to_csv(rf"C:\Users\rudkevich-k\Desktop\patent_diploma\tests_searching\result\result_{mode}_{args.experiment}.csv")
